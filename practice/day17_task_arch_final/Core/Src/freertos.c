  ...
  /* Infinite loop */
    ...
    for(;;)
    {
        ...
        if (sensor_snapshot.ready != 0U)
        {
            ...
            {
							sprintf(uart_buf, "DHT11 TEMP:%d.%dC HUMI:%d.%d%%CNT:%lu\r\n",
                        sensor_snapshot.temp_int,
                        sensor_snapshot.temp_dec,
                        sensor_snapshot.humi_int,
                        sensor_snapshot.humi_dec,sensor_snapshot.update_count);
            }
            else
            {
							sprintf(uart_buf, "DHT11 READ ERROR  CNT:%lu ERR:%lu \r\n",sensor_snapshot.update_count,sensor_snapshot.error_count);
            }
