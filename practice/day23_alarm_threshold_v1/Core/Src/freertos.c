...
static AlarmState CheckAlarmState(uint8_t temp_int,uint8_t humi_int);
...
void MX_FREERTOS_Init(void) {
  ...
  /* definition and creation of ledTask */
	osThreadStaticDef(displayTask, StartDisplayTask, osPriorityNormal, 0, 256,
										displayTaskStack, &displayTaskControlBlock);
  ...
  /* definition and creation of uartTask */
	osThreadStaticDef(uartTask, StartUartTask, osPriorityNormal, 0, 256,
										uartTaskStack, &uartTaskControlBlock);
  ...
  /* definition and creation of sensorTask */
		osThreadStaticDef(sensorTask, StartSensorTask, osPriorityAboveNormal, 0, 256,
										sensorTaskStack, &sensorTaskControlBlock);
...
//						HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_13);
        ...
        if (msg.valid)
        {
					...
					sprintf(uart_buf, "DHT11 TEMP:%d.%dC HUMI:%d.%d%% CNT:%lu ERR:%lu ALARM:%s\r\n",
              msg.temp_int,
              msg.temp_dec,
              msg.humi_int,
              msg.humi_dec,
  ...
  for(;;)
  {
        if (DHT11_ReadData(&dht11_data) == HAL_OK)				//��ȡ�ɹ�������HAL_OK
        {
            ...
            g_sensor_data.valid = 1;
						...
						g_sensor_data.alarm_state=CheckAlarmState(g_sensor_data.temp_int,g_sensor_data.humi_int);
        ...
        osDelay(g_app_config.sample_period_ms);
  }
